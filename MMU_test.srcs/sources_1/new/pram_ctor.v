...
/*******************************  内存回收  *****************************/
localparam S_IDLE_RECLAIM    = 5'b0_0001;                  // 空闲状态
localparam S_ARBI_RECLAIM    = 5'b0_0010;                  // 处理多端口同时申请的仲裁状态
localparam S_READ_RECLAIM    = 5'b0_0100;                  // 读sram状态
localparam S_RECLAIM         = 5'b0_1000;                  // 内存回收状态
localparam S_LOAD_FIRST_ADDR = 5'b1_0000;                  // 加载读取首地址
...
always @(posedge i_clk or negedge i_rst_n) begin
    if (~i_rst_n) begin 
        ...
        read_num <= i_pd[6:0];
        portb_rd_cnt <= i_pd[6:0] - 1;
        rd_first_addr <= i_pd[17:7];
    end
